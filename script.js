...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (roomFromUrl) {
        ...
        // ✅ FIX: ตรวจสอบว่าห้องยังอยู่จริงก่อน restore session
        restoreRoomSession(savedRoom);
    }
});

// ===== FIX: Restore session with room existence check =====
async function restoreRoomSession(roomId) {
    try {
        const snap = await getDoc(doc(db, "rooms", roomId, "meta", "settings"));
        if (!snap.exists()) {
            // ห้องหายไปแล้ว (ถูกลบ หรือไม่เคยมี) → ล้าง localStorage แล้วกลับหน้าแรก
...
async function handleJoinClick() {
    ...
    if (!name) {
        showBubbleNotification("⚠️ ยังไม่ได้กรอกชื่อ", "กรุณาระบุชื่อเล่นของคุณก่อนครับ", "warning");
        return;
    }
    if (code.length !== 4 || isNaN(code)) {
        showBubbleNotification("⚠️ รหัสห้องไม่ถูกต้อง", "รหัสห้องต้องเป็นตัวเลข 4 หลักเท่านั้นครับ", "warning");
        return;
    }
    ...
    try {
        ...
        // ✅ FIX: ถ้าไม่มี doc → ห้องไม่มีจริง → แสดง popup กลางจอ
        if (!roomMeta.exists()) {
            showRoomNotFoundOverlay(code);
            return;
        }

        if (roomMeta.data().hasPassword) {
            showAuthScreen_Join();
        } else {
            enterRoom(code);
        }
...
async function handleConfirmCreate() {
    ...
    try {
        ...
        await setDoc(settingsRef, usePassword
            ? { hasPassword: true, password: pwd, owner: myUsername, lastActivity: new Date() }
...
function renderMemberChips(snapshot) {
    ...
    snapshot.forEach((d) => {
        ...
        if (isMe) label += ' (คุณ)';
        let ownerTag = isMemberOwner ? `<span class="owner-tag">owner</span>` : '';
        let kickBtn = '';
...
// ===== FIX: Leave Room — Owner ต้องลบห้องก่อนออก =====
function handleLeaveClick() {
    if (myUsername === roomOwner) {
        // แสดง dialog พิเศษสำหรับ owner
        showOwnerLeaveDialog();
    } else {
        // สมาชิกทั่วไป — ออกได้ปกติ
        showBubbleConfirm(
            "ออกจากห้องนี้?",
            "ข้อมูลการบ้านยังคงอยู่สำหรับสมาชิกคนอื่น",
            () => leaveRoom()
        );
    }
}
...
async function deleteRoomAndLeave() {
    ...
    try {
        ...
        console.error("ลบห้องไม่สำเร็จ:", err);
    }
...
window.openCommentPanel = function(taskId) {
    ...
    const commentsRef = collection(db, "rooms", currentRoomId, "assignments", taskId, "comments");
...
async function submitComment() {
    ...
    input.value = '';
    try {
        await addDoc(collection(db, "rooms", currentRoomId, "assignments", currentCommentTaskId, "comments"), {
            text,
            author: myUsername,
...
function openModal(mode, id = null) {
    ...
    const linkField = document.getElementById('task-link-wrap');
...
async function saveAssignmentToServer(e) {
    ...
    let link = '';
    if (linkRaw) {
        link = linkRaw.startsWith('http') ? linkRaw : 'https://' + linkRaw;
    }
    try {
        await setDoc(doc(db, "rooms", currentRoomId, "assignments", id), {
            id, subject, title, details, dueDate, link,
